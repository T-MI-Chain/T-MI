...
# Check for any changes in any runtime directories (e.g., ^runtime/tmi) as
...
SUBSTRATE_REPO="https://github.com/tmi/substrate"
...
please make sure versions are bumped in tmi accordingly
EOT
  ...
  (
    ...
    echo "$substrate_changes" | while read -r line; do
      ...
      if has_label 'tmi/substrate' "$pr_id" 'D2-breaksapi'; then
