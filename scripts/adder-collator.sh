...
# disabled until we can actually successfully register the chain with tmi-js-api
# if ! command -v tmi-js-api > /dev/null; then
#   echo "tmi-js-api required; try"
#   echo "  sudo yarn global add @tmi/api-cli"
#   exit 1
# fi
...
tmi="target/release/tmi"
adder_collator="target/release/adder-collator"

# ensure the tmi binary exists and is up to date
if [ ! -x "$tmi" ] || [ "$tmi" -ot "$last_modified_rust_file" ]; then
...
# The name must be one of those which can be passed to the tmi binary, in un-flagged form,
# one of:
...
# doesn't work yet due to https://github.com/tmi-js/tools/issues/185
# tmi-js-api \
