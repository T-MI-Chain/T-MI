...
impl SubstrateCli for Cli {
	fn impl_name() -> String { "Parity tmi".into() }
	...
	fn support_url() -> String { "https://github.com/tmi/tmi/issues/new".into() }

	fn copyright_start_year() -> i32 { 2017 }

	fn executable_name() -> String { "tmi".into() }

	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		let id = if id == "" {
			let n = get_exec_name().unwrap_or_default();
			["tmi", "kusama", "westend", "rococo"].iter()
				.cloned()
				.find(|&chain| n.starts_with(chain))
				.unwrap_or("tmi")
		} else { id };
		Ok(match id {
			"tmi-dev" | "dev" => Box::new(service::chain_spec::tmi_development_config()?),
			"tmi-local" => Box::new(service::chain_spec::tmi_local_testnet_config()?),
			"tmi-staging" => Box::new(service::chain_spec::tmi_staging_testnet_config()?),
			"kusama-dev" => Box::new(service::chain_spec::kusama_development_config()?),
			"kusama-local" => Box::new(service::chain_spec::kusama_local_testnet_config()?),
			"kusama-staging" => Box::new(service::chain_spec::kusama_staging_testnet_config()?),
			"tmi" => Box::new(service::chain_spec::tmi_config()?),
			...
			path => {
				...
				if self.run.force_rococo || starts_with("rococo") {
					...
					Box::new(service::tmiChainSpec::from_json_file(path)?)
				}
			},
		})
	}
...
fn set_default_ss58_version(spec: &Box<dyn service::ChainSpec>) {
	...
	let ss58_version = if spec.is_kusama() {
		...
		Ss58AddressFormat::tmiAccount
	};

	sp_core::crypto::set_default_ss58_version(ss58_version);
}

/// Parses tmi specific CLI arguments and run the service.
pub fn run() -> Result<()> {
	...
	match &cli.subcommand {
		...
		Some(Subcommand::ValidationWorker(cmd)) => {
			...
			if cfg!(feature = "browser") || cfg!(target_os = "android") {
				...
				tmi_parachain::wasm_executor::run_worker(&cmd.mem_id)?;
				Ok(())
			}
		},
