...
//! The collation generation subsystem is the interface between tmi and the collators.
...
use tmi_node_primitives::CollationGenerationConfig;
use tmi_node_subsystem::{
	messages::{AllMessages, CollationGenerationMessage, CollatorProtocolMessage},
	FromOverseer, SpawnedSubsystem, Subsystem, SubsystemContext, SubsystemResult,
};
use tmi_node_subsystem_util::{
...
impl CollationGenerationSubsystem {
	...
	where
		Context: SubsystemContext<Message = CollationGenerationMessage>,
	{
		use tmi_node_subsystem::ActiveLeavesUpdate;
		use tmi_node_subsystem::FromOverseer::{Communication, Signal};
		use tmi_node_subsystem::OverseerSignal::{ActiveLeaves, BlockFinalized, Conclude};
		...
		for (core_idx, core) in availability_cores.into_iter().enumerate() {
			...
			let (scheduled_core, assumption) = match core {
				...
				CoreState::Occupied(_occupied_core) => {
					// TODO: https://github.com/tmi/tmi/issues/1573
					tracing::trace!(
						target: LOG_TARGET,
						core_idx = %core_idx,
...
fn erasure_root(
	...
	let chunks = tmi_erasure_coding::obtain_chunks_v1(n_validators, &available_data)?;
	Ok(tmi_erasure_coding::branches(&chunks).root())
}
...
mod tests {
	mod handle_new_activations {
		...
		use tmi_node_primitives::Collation;
		use tmi_node_subsystem::messages::{
