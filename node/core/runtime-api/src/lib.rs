...
use tmi_primitives::v1::{Block, BlockId, Hash, ParachainHost};
...
const API_REQUEST_TASK_NAME: &str = "tmi-runtime-api-request";
...
mod tests {
	...
	sp_api::mock_impl_runtime_apis! {
		impl ParachainHost<Block> for MockRuntimeApi {
			...
			fn check_validation_outputs(
				&self,
				para_id: ParaId,
				_commitments: tmi_primitives::v1::CandidateCommitments,
			) -> bool {
	...
	fn requests_check_validation_outputs() {
		...
		let para_a = 5.into();
		let para_b = 6.into();
		let commitments = tmi_primitives::v1::CandidateCommitments::default();
		let spawner = sp_core::testing::TaskExecutor::new();
