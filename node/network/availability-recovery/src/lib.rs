...
//! Availability Recovery Subsystem of tmi.
...
use tmi_erasure_coding::{branches, branch_hash, recovery_threshold, obtain_chunks_v1};
mod error;
	...
	async fn run(mut self) -> error::Result<()> {
		loop {
			...
			if self.received_chunks.len() >= self.threshold {
				let concluded = match tmi_erasure_coding::reconstruct_v1(
					self.validators.len(),
