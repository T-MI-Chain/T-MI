...
use tmi_node_network_protocol::{v1 as protocol_v1, View, PeerId, RequestId, OurView};
...
mod tests {
	...
	use tmi_subsystem::{ActiveLeavesUpdate, messages::{RuntimeApiMessage, RuntimeApiRequest}, JaegerSpan};
	use tmi_node_subsystem_util::TimeoutExt;
	use tmi_subsystem_testhelpers as test_helpers;
	use tmi_node_network_protocol::{view, our_view};
		...
		overseer_send(
			...
			CollatorProtocolMessage::NetworkBridgeUpdateV1(
				NetworkBridgeEvent::PeerConnected(
					peer.clone(),
					tmi_node_network_protocol::ObservedRole::Authority,
				),
			),
		).await;
