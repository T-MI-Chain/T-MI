...
use tmi_primitives::v1::{Block, Hash};
use tmi_subsystem::{SubsystemError, SubsystemResult};
...
pub trait Network: Send + 'static {
	...
	/// to the tmi protocol - the user of this function should filter only for events related
...
impl Network for Arc<NetworkService<Block, Hash>> {
	fn event_stream(&mut self) -> BoxStream<'static, NetworkEvent> {
		NetworkService::event_stream(self, "tmi-network-bridge").boxed()
	}
