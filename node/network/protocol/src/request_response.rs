...
//! Overview over request/responses as used in `tmi`.
//!
...
impl Protocol {
	...
	pub fn get_config(
		...
		let cfg = match self {
			Protocol::AvailabilityFetching => RequestResponseConfig {
				...
				// TODO: Get better numbers, see https://github.com/tmi/tmi/issues/2370
	...
	pub const fn get_protocol_name_static(self) -> &'static str {
		match self {
			Protocol::AvailabilityFetching => "/tmi/req_availability/1",
		}
	}
}
