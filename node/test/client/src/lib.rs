...
//! A tmi test client.
//!
//! This test client is using the tmi test runtime.
...
pub use tmi_test_service::{
	Client, construct_extrinsic, construct_transfer_extrinsic, tmiTestExecutor, FullBackend,
};
pub use tmi_test_runtime as runtime;

/// Test client executor.
pub type Executor = client::LocalCallExecutor<FullBackend, sc_executor::NativeExecutor<tmiTestExecutor>>;

/// Test client builder for tmi.
...
impl substrate_test_client::GenesisInit for GenesisParameters {
	fn genesis_storage(&self) -> Storage {
		tmi_test_service::chain_spec::tmi_local_testnet_genesis()
			.build_storage()
...
mod tests{
	...
	fn ensure_test_client_can_build_and_import_block() {
		...
		let block_builder = client.init_tmi_block_builder();
	...
	fn ensure_test_client_can_push_extrinsic() {
		...
		let mut block_builder = client.init_tmi_block_builder();
		block_builder.push_tmi_extrinsic(transfer).expect("Pushes extrinsic");
