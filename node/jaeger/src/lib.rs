...
//! tmi Jaeger related primitives
//!
//! Provides primitives used by tmi for interfacing with Jaeger.
//!
//! # Integration
//!
...
use tmi_primitives::v1::{Hash, PoV, CandidateHash};
...
impl Jaeger {
	...
	pub fn launch<S: SpawnNamed>(self, spawner: S) -> result::Result<(), JaegerError> {
		...
		let (traces_in, mut traces_out) = mick_jaeger::init(mick_jaeger::Config {
			service_name: format!("tmi-{}", cfg.node_name),
		});
