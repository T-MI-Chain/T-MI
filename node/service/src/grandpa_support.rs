...
//! tmi-specific GRANDPA integration utilities.
...
pub(crate) fn kusama_hard_forks() -> Vec<(
	...
	(Hash, tmi_primitives::v1::BlockNumber),
...
mod tests {
	...
	use tmi_test_client::{
		TestClientBuilder, TestClientBuilderExt, DefaultTestClientBuilderExt, InittmiBlockBuilder,
	...
	fn grandpa_pause_voting_rule_works() {
		...
		let mut push_blocks = {
			...
			move |n| {
				for _ in 0..n {
					let block = client.init_tmi_block_builder().build().unwrap().block;
