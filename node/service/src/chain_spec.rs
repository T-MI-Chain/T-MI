...
use tmi::constants::currency::DOTS;
use tmi_primitives::v1::{AccountId, AccountPublic, ValidatorId, AssignmentId};
use tmi_runtime as tmi;
...
const tmi_STAGING_TELEMETRY_URL: &str = "wss://telemetry.tmi.io/submit/";
const KUSAMA_STAGING_TELEMETRY_URL: &str = "wss://telemetry.tmi.io/submit/";
const WESTEND_STAGING_TELEMETRY_URL: &str = "wss://telemetry.tmi.io/submit/";
const ROCOCO_STAGING_TELEMETRY_URL: &str = "wss://telemetry.tmi.io/submit/";
...
pub struct Extensions {
	...
	pub fork_blocks: sc_client_api::ForkBlocks<tmi_primitives::v1::Block>,
	/// Known bad block hashes.
	pub bad_blocks: sc_client_api::BadBlocks<tmi_primitives::v1::Block>,
}

/// The `ChainSpec` parametrised for the tmi runtime.
pub type tmiChainSpec = service::GenericChainSpec<tmi::GenesisConfig, Extensions>;
...
pub fn tmi_config() -> Result<tmiChainSpec, String> {
	tmiChainSpec::from_json_bytes(&include_bytes!("../res/tmi.json")[..])
}
...
pub fn westend_config() -> Result<tmiChainSpec, String> {
	tmiChainSpec::from_json_bytes(&include_bytes!("../res/westend.json")[..])
}

pub fn rococo_config() -> Result<tmiChainSpec, String> {
	tmiChainSpec::from_json_bytes(&include_bytes!("../res/rococo.json")[..])
}
...
fn tmi_staging_testnet_config_genesis(wasm_binary: &[u8]) -> tmi::GenesisConfig {
	...
	tmi::GenesisConfig {
		frame_system: Some(tmi::SystemConfig {
			code: wasm_binary.to_vec(),
			changes_trie_config: Default::default(),
		}),
		pallet_balances: Some(tmi::BalancesConfig {
		...
		pallet_indices: Some(tmi::IndicesConfig { indices: vec![] }),
		pallet_session: Some(tmi::SessionConfig {
		...
		pallet_staking: Some(tmi::StakingConfig {
			validator_count: 50,
		...
		pallet_collective_Instance1: Some(tmi::CouncilConfig {
			members: vec![],
			phantom: Default::default(),
		}),
		pallet_collective_Instance2: Some(tmi::TechnicalCommitteeConfig {
			members: vec![],
			phantom: Default::default(),
		}),
		...
		pallet_authority_discovery: Some(tmi::AuthorityDiscoveryConfig { keys: vec![] }),
		claims: Some(tmi::ClaimsConfig {
			claims: vec![],
			vesting: vec![],
		}),
		pallet_vesting: Some(tmi::VestingConfig { vesting: vec![] }),
...
fn rococo_staging_testnet_config_genesis(wasm_binary: &[u8]) -> rococo_runtime::GenesisConfig {
	...
	rococo_runtime::GenesisConfig {
		...
		parachains_configuration: Some(rococo_runtime::ParachainsConfigurationConfig {
			config: tmi_runtime_parachains::configuration::HostConfiguration {
...
/// tmi staging testnet config.
pub fn tmi_staging_testnet_config() -> Result<tmiChainSpec, String> {
	let wasm_binary = tmi::WASM_BINARY.ok_or("tmi development wasm not available")?;
	let boot_nodes = vec![];

	Ok(tmiChainSpec::from_genesis(
		"tmi Staging Testnet",
		"tmi_staging_testnet",
		ChainType::Live,
		move || tmi_staging_testnet_config_genesis(wasm_binary),
		boot_nodes,
		Some(
			TelemetryEndpoints::new(vec![(tmi_STAGING_TELEMETRY_URL.to_string(), 0)])
				.expect("tmi Staging telemetry url is valid; qed"),
		),
...
/// Helper function to create tmi GenesisConfig for testing
pub fn tmi_testnet_genesis(
	wasm_binary: &[u8],
	...
	tmi::GenesisConfig {
		...
		pallet_elections_phragmen: Some(Default::default()),
		pallet_democracy: Some(tmi::DemocracyConfig::default()),
		pallet_collective_Instance1: Some(tmi::CouncilConfig {
			members: vec![],
			phantom: Default::default(),
		}),
...
fn tmi_development_config_genesis(wasm_binary: &[u8]) -> tmi::GenesisConfig {
	tmi_testnet_genesis(
		wasm_binary,
...
/// tmi development config (single validator Alice)
pub fn tmi_development_config() -> Result<tmiChainSpec, String> {
	let wasm_binary = tmi::WASM_BINARY.ok_or("tmi development wasm not available")?;
	...
	Ok(tmiChainSpec::from_genesis(
		...
		move || tmi_development_config_genesis(wasm_binary),
		vec![],
		None,
		Some(DEFAULT_PROTOCOL_ID),
		None,
...
fn tmi_local_testnet_genesis(wasm_binary: &[u8]) -> tmi::GenesisConfig {
	tmi_testnet_genesis(
		wasm_binary,
		vec![
...
/// tmi local testnet config (multivalidator Alice + Bob)
pub fn tmi_local_testnet_config() -> Result<tmiChainSpec, String> {
	let wasm_binary = tmi::WASM_BINARY.ok_or("tmi development wasm not available")?;
	...
	Ok(tmiChainSpec::from_genesis(
		...
		"local_testnet",
		ChainType::Local,
		move || tmi_local_testnet_genesis(wasm_binary),
		vec![],
		None,
		Some(DEFAULT_PROTOCOL_ID),
		None,
