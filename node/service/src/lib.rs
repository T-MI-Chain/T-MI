...
//! tmi service. Specialized wrapper over substrate service.
...
use {
	...
	std::time::Duration,
	tracing::info,
	tmi_node_core_av_store::Config as AvailabilityConfig,
	tmi_node_core_av_store::Error as AvailabilityError,
	tmi_node_core_proposer::ProposerFactory,
	tmi_overseer::{AllSubsystems, BlockInfo, Overseer, OverseerHandler},
	tmi_primitives::v1::ParachainHost,
...
use tmi_network_bridge::RequestMultiplexer;

use sp_core::traits::SpawnNamed;


use tmi_subsystem::jaeger;
...
pub use chain_spec::{tmiChainSpec, KusamaChainSpec, WestendChainSpec, RococoChainSpec};
pub use consensus_common::{Proposal, SelectChain, BlockImport, RecordProof, block_validation::Chain};
pub use tmi_parachain::wasm_executor::IsolationStrategy;
pub use tmi_primitives::v1::{Block, BlockId, CollatorId, Hash, Id as ParaId};
...
native_executor_instance!(
	pub tmiExecutor,
	tmi_runtime::api::dispatch,
	tmi_runtime::native_version,
...
pub enum Error {
	...
	Overseer(#[from] tmi_overseer::SubsystemError),

	#[error(transparent)]
	Prometheus(#[from] prometheus_endpoint::PrometheusError),

	#[error(transparent)]
	Jaeger(#[from] tmi_subsystem::jaeger::JaegerError),
...
// If we're using prometheus, use a registry with a prefix of `tmi`.
fn set_prometheus_registry(config: &mut Configuration) -> Result<(), Error> {
	if let Some(PrometheusConfig { registry, .. }) = config.prometheus_config.as_mut() {
		*registry = Registry::new_custom(Some("tmi".into()), None)?;
	}
...
fn new_partial<RuntimeApi, Executor>(config: &mut Configuration, jaeger_agent: Option<std::net::SocketAddr>) -> Result<
	service::PartialComponents<
		...
		(
			impl Fn(
				tmi_rpc::DenyUnsafe,
				tmi_rpc::SubscriptionTaskExecutor,
			) -> tmi_rpc::RpcExtension,
			(
		...
		Executor: NativeExecutionDispatch + 'static,
{
	...
	let rpc_extensions_builder = {
		...
		move |deny_unsafe, subscription_executor| -> tmi_rpc::RpcExtension {
			let deps = tmi_rpc::FullDeps {
				client: client.clone(),
				pool: transaction_pool.clone(),
				select_chain: select_chain.clone(),
				chain_spec: chain_spec.cloned_box(),
				deny_unsafe,
				babe: tmi_rpc::BabeDeps {
					babe_config: babe_config.clone(),
					shared_epoch_changes: shared_epoch_changes.clone(),
					keystore: keystore.clone(),
				},
				grandpa: tmi_rpc::GrandpaDeps {
...
where
	...
	Spawner: 'static + SpawnNamed + Clone + Unpin,
{
	use tmi_node_subsystem_util::metrics::Metrics;

	use tmi_availability_distribution::AvailabilityDistributionSubsystem;
	use tmi_node_core_av_store::AvailabilityStoreSubsystem;
	use tmi_availability_bitfield_distribution::BitfieldDistribution as BitfieldDistributionSubsystem;
	use tmi_node_core_bitfield_signing::BitfieldSigningSubsystem;
	use tmi_node_core_backing::CandidateBackingSubsystem;
	use tmi_node_core_candidate_selection::CandidateSelectionSubsystem;
	use tmi_node_core_candidate_validation::CandidateValidationSubsystem;
	use tmi_node_core_chain_api::ChainApiSubsystem;
	use tmi_node_collation_generation::CollationGenerationSubsystem;
	use tmi_collator_protocol::{CollatorProtocolSubsystem, ProtocolSide};
	use tmi_network_bridge::NetworkBridge as NetworkBridgeSubsystem;
	use tmi_pov_distribution::PoVDistribution as PoVDistributionSubsystem;
	use tmi_node_core_provisioner::ProvisioningSubsystem as ProvisionerSubsystem;
	use tmi_node_core_runtime_api::RuntimeApiSubsystem;
	use tmi_statement_distribution::StatementDistribution as StatementDistributionSubsystem;
	use tmi_availability_recovery::AvailabilityRecoverySubsystem;
	use tmi_approval_distribution::ApprovalDistribution as ApprovalDistributionSubsystem;
		...
		Executor: NativeExecutionDispatch + 'static,
{
	...
	// Note: GrandPa is pushed before the tmi-specific protocols. This doesn't change
	// anything in terms of behaviour, but makes the logs more consistent with the other
	// Substrate nodes.
	config.network.extra_sets.push(grandpa::grandpa_peers_set_config());
	#[cfg(feature = "real-overseer")]
	config.network.extra_sets.extend(tmi_network_bridge::peer_sets_info());
	...
	// Tracking issue: https://github.com/tmi/tmi/issues/2283
	#[cfg(feature = "real-overseer")]
	...
	let overseer_handler = if let Some(authority_discovery_service) = authority_discovery_service {
		...
		task_manager.spawn_essential_handle().spawn_blocking("overseer", Box::pin(async move {
			...
			let forward = tmi_overseer::forward_events(overseer_client, overseer_handler_clone);
		...
		Dispatch: NativeExecutionDispatch + 'static,
{
	...
	let light_deps = tmi_rpc::LightDeps {
	...
	let rpc_extensions = tmi_rpc::create_light(light_deps);
...
>
{
	...
	if config.chain_spec.is_rococo() {
		...
		let service::PartialComponents { client, backend, import_queue, task_manager, .. }
			= new_partial::<tmi_runtime::RuntimeApi, tmiExecutor>(config, jaeger_agent)?;
		Ok((Arc::new(Client::tmi(client)), backend, import_queue, task_manager))
	}
}
...
pub fn build_light(config: Configuration) -> Result<(
	...
	if config.chain_spec.is_rococo() {
		...
		new_light::<tmi_runtime::RuntimeApi, tmiExecutor>(config)
	}
}
...
pub fn build_full(
	...
	if config.chain_spec.is_rococo() {
		...
		new_full::<tmi_runtime::RuntimeApi, tmiExecutor>(
			config,
			is_collator,
			grandpa_pause,
			jaeger_agent,
			Default::default(),
		).map(|full| full.with_client(Client::tmi))
	}
}
