...
use tmi_primitives::v1::{Block, ParachainHost, AccountId, Nonce, Balance, Header, BlockNumber, Hash};
use consensus_common::BlockStatus;

/// A set of APIs that tmi-like runtimes must implement.
...
///
/// As there exist multiple chains inside tmi, like tmi itself, Kusama, Westend etc,
...
/// A handle to a tmi client instance.
///
/// The tmi service supports multiple different runtimes (Westend, tmi itself, etc). As each runtime has a
...
/// A client instance of tmi.
///
/// See [`ExecuteWithClient`] for more information.
#[derive(Clone)]
pub enum Client {
	tmi(Arc<crate::FullClient<tmi_runtime::RuntimeApi, crate::tmiExecutor>>),
...
impl sc_client_api::UsageProvider<Block> for Client {
	fn usage_info(&self) -> sc_client_api::ClientInfo<Block> {
		match self {
			Self::tmi(client) => client.usage_info(),
...
impl sc_client_api::BlockBackend<Block> for Client {
	fn block_body(
		...
		match self {
			Self::tmi(client) => client.block_body(id),
	...
	fn block(&self, id: &BlockId<Block>) -> sp_blockchain::Result<Option<SignedBlock<Block>>> {
		match self {
			Self::tmi(client) => client.block(id),
	...
	fn block_status(&self, id: &BlockId<Block>) -> sp_blockchain::Result<BlockStatus> {
		match self {
			Self::tmi(client) => client.block_status(id),
	...
	fn justification(
		...
		match self {
			Self::tmi(client) => client.justification(id),
	...
	fn block_hash(
		...
		match self {
			Self::tmi(client) => client.block_hash(number),
	...
	fn extrinsic(
		...
		match self {
			Self::tmi(client) => client.extrinsic(id),
...
impl sc_client_api::StorageProvider<Block, crate::FullBackend> for Client {
	fn storage(
		...
		match self {
			Self::tmi(client) => client.storage(id, key),
	...
	fn storage_keys(
		...
		match self {
			Self::tmi(client) => client.storage_keys(id, key_prefix),
	...
	fn storage_hash(
		...
		match self {
			Self::tmi(client) => client.storage_hash(id, key),
	...
	fn storage_pairs(
		...
		match self {
			Self::tmi(client) => client.storage_pairs(id, key_prefix),
	...
	fn storage_keys_iter<'a>(
		...
		match self {
			Self::tmi(client) => client.storage_keys_iter(id, prefix, start_key),
	...
	fn child_storage(
		...
		match self {
			Self::tmi(client) => client.child_storage(id, child_info, key),
	...
	fn child_storage_keys(
		...
		match self {
			Self::tmi(client) => client.child_storage_keys(id, child_info, key_prefix),
	...
	fn child_storage_hash(
		...
		match self {
			Self::tmi(client) => client.child_storage_hash(id, child_info, key),
	...
	fn max_key_changes_range(
		...
		match self {
			Self::tmi(client) => client.max_key_changes_range(first, last),
	...
	fn key_changes(
		...
		match self {
			Self::tmi(client) => client.key_changes(first, last, storage_key, key),
...
impl sp_blockchain::HeaderBackend<Block> for Client {
	fn header(&self, id: BlockId<Block>) -> sp_blockchain::Result<Option<Header>> {
		match self {
			Self::tmi(client) => client.header(&id),
	...
	fn status(&self, id: BlockId<Block>) -> sp_blockchain::Result<sp_blockchain::BlockStatus> {
		match self {
			Self::tmi(client) => client.status(id),
	...
	fn number(&self, hash: Hash) -> sp_blockchain::Result<Option<BlockNumber>> {
		match self {
			Self::tmi(client) => client.number(hash),
	...
	fn hash(&self, number: BlockNumber) -> sp_blockchain::Result<Option<Hash>> {
		match self {
			Self::tmi(client) => client.hash(number),
