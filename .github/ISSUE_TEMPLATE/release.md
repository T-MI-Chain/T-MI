...
title: tmi {{ env.VERSION }} Release checklist
---

# Release Checklist

This is the release checklist for tmi {{ env.VERSION }}. **All** following
checks should be completed before publishing a new release of the
tmi/Kusama/Westend runtime or client. The current release candidate can be
...
- [ ] Verify [new migrations](#new-migrations) complete successfully, and the
      ...
      networks.
- [ ] Verify [tmi JS API](#tmi-js) are up to date with the latest
      runtime changes.
...
- [ ] Check that a draft release has been created at
      https://github.com/tmi/tmi/releases with relevant [release
      notes](#release-notes)
...
tmi validators for at least 12 hours prior to publishing the release.
...
1. Download the latest release-candidate binary either from the draft-release
   on Github, or
   [AWS](https://releases.parity.io/tmi/x86_64-debian:stretch/{{ env.VERSION }}-rc1/tmi)
   (adjust the rc in this URL as necessary).
2. Run the release-candidate binary using a local chain:
   `./tmi --chain=tmi-local` or `./tmi --chain=kusama.local`
3. Use [`tmi-js-tools`](https://github.com/tmi-js/tools) to compare
   the metadata:

- For tmi: `docker run --network host jacogr/tmi-js-tools metadata wss://rpc.tmi.io ws://localhost:9944`
- For Kusama: `docker run --network host jacogr/tmi-js-tools metadata wss://kusama-rpc.tmi.io ws://localhost:9944`
...
Ensure that a release of [tmi JS API]() contains any new types or
